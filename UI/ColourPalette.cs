...
public static class ColourPalette
{
    ...
    public static string GetCircadianThemeName()
    {
        ...
        if (time >= new TimeSpan(5, 0, 0) && time < new TimeSpan(8, 30, 0))
        {
            return "DAWN // AMBER MIST";
        }
        if (time >= new TimeSpan(8, 30, 0) && time < new TimeSpan(17, 0, 0))
        {
            return "ZENITH // HIGH CANOPY";
        }
        if (time >= new TimeSpan(17, 0, 0) && time < new TimeSpan(20, 0, 0))
        {
            return "GOLDEN HOUR // SAFFRON HARVEST";
        }
