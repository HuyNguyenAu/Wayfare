...
internal sealed class ExecuteCommandTool(IToolHelpers toolHelpers) : ITool
{
    ...
    {
        return _toolHelpers.TryDeserialiseArguments(arguments, out ExecuteCommandArguments? commandArguments, out _)
    ...
    {
        if (!_toolHelpers.TryDeserialiseArguments(arguments, out ExecuteCommandArguments? executeCommandArguments, out string? executeCommandArgumentsError))
        {
        ...
        {
            return new ToolExecutionResult(false, $"An unexpected error occurred while executing command '{executeCommandArguments.Command}'.", string.Empty, $"Failed to execute command: an unexpected error occurred. Error: {exception.Message}");
        }
    }
