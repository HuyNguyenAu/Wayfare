...
internal sealed class ReadFileTool(IToolHelpers toolHelpers) : ITool
{
    ...
    public ToolSchema Parameters => ToolSchema.Object(new Dictionary<string, ToolPropertySchema>
    {
        ...
        ["limit"] = ToolPropertySchema.Integer("Maximum number of lines to return. Defaults to 200.")
    });

    public string GetInvocationMessage(string arguments)
    {
        return _toolHelpers.TryDeserialiseArguments(arguments, out ReadFileArguments? parsedArguments, out _)
    ...
    {
        if (!_toolHelpers.TryDeserialiseArguments(arguments, out ReadFileArguments? readFileArguments, out string? readFileArgumentsError))
        {
            return new ToolExecutionResult(false, "Failed to read file due to invalid tool arguments.", string.Empty, $"Failed to read file: invalid tool arguments. Error: {readFileArgumentsError}. Usage: {{\"path\": \"<file_path>\", \"offset\": 0, \"limit\": 200}}");
        }

        if (readFileArguments.Offset < 0)
        {
            return new ToolExecutionResult(false, "Failed to read file because 'offset' cannot be negative.", string.Empty, "Failed to read file: 'offset' parameter must be non-negative (0 or greater). Usage: {\"path\": \"<path>\", \"offset\": 0, \"limit\": 200}.");
        }

        if (readFileArguments.Limit < 0)
        {
            return new ToolExecutionResult(false, "Failed to read file because 'limit' cannot be negative.", string.Empty, "Failed to read file: 'limit' parameter must be non-negative. Usage: {\"path\": \"<path>\", \"offset\": 0, \"limit\": 200}.");
        }

        if (!_toolHelpers.TryGetRequiredPath(readFileArguments.Path, out string? resolvedPath, out string? requiredPathError))
        {
        ...
        try
        {
            ...
            string[] numberedLines = [.. lineSlice.Select((line, index) => $"{readFileArguments.Offset + index + 1,5} | {line}")];
            string result = $"""
            <observation tool="read_file" path="{readFileArguments.Path}" lines="{readFileArguments.Offset + 1}-{readFileArguments.Offset + lineSlice.Length}" total_lines="{lines.Length}">
            {string.Join(Environment.NewLine, numberedLines)}
            </observation>
            """;
        ...
        {
            return new ToolExecutionResult(false, $"An unexpected error occurred while reading file '{readFileArguments.Path}'.", string.Empty, $"Failed to read file: an unexpected error occurred. Error: {exception.Message}");
        }
    }

    internal record ReadFileArguments(string Path = "", int Offset = 0, int Limit = 200);
}
