...
internal sealed class ReplaceTool(IToolHelpers toolHelpers) : ITool
{
    ...
    public async Task<ToolExecutionResult> ExecuteAsync(string arguments, CancellationToken cancellationToken)
    {
        ...
        {
            return new ToolExecutionResult(false, string.Empty, replaceArgumentsError);
        }

        if (!toolHelpers.TryGetRequiredPath(replaceArguments.Path, out string? resolvedPath, out string? requiredPathError))
        {
            return new ToolExecutionResult(false, string.Empty, $"Failed to replace text because {requiredPathError}");
        }

        if (!File.Exists(resolvedPath))
        {
            return new ToolExecutionResult(false, string.Empty, $"Failed to replace text because file does not exist at path '{replaceArguments.Path}'");
        }
